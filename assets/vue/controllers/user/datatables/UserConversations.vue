...
<template>
    <VDatatable :searchable-properties="['messages']" :exclude-from-row-properties="['id', 'messages', 'isArchivedByUser']"
        ...
        <template #rowHeader="{ item }">
            <span class="badge fw-bold fs-5 rounded-pill me-3"
        ...
        <template #buttons="{ item }">
            <Button label="Open" class="my-1" color-class="primary"
                @click.prevent="goTo(`/user/message/${item.messages[0].id}/show`)" icon-class-end="box-arrow-up-right" />
            <Button label="Archive" class="my-1" color-class="warning" icon-class-end="box-fill"
