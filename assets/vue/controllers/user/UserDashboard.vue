...
<template>
    ...
    <VDatatable title="My current reservations" :searchable-properties="[
        'reservationNumber'
    ]" :data="reservations" :exclude-from-row-properties="[
    'id',
    'adultCount',
    'childCount',
    'updatedOn'
]" :date-filter="{ label: 'created on', codeName: 'createdOn' }" hide-order-by hide-empty>
        <template #titleButtons>
            <Button label="Send us a message" icon-class-start="send-plus-fill" data-bs-toggle="modal"
                data-bs-target="#userNewMessage" />
        </template>
        <template #buttons="{ item }">
            <Button label="Open" class="my-1" color-class="primary"
                @click.prevent="goTo(`/user/reservation/${item.id}/show`)" icon-class-end="box-arrow-up-right" />
            <Button label="Ask a question..." class="my-1 text-white" color-class="info"
