  ...
  <section class="row row-cols-2">
    ...
    >
      <template #notification="{ item }">
        <a
          class="fw-bold icon-link"
          :href="`${userBaseUrl}/${item.user.id}/show`"
          >{{ item.user.value }}</a
        >
        ...
        <div class="d-inline" v-if="item.lodgings.length > 1">
          <div
            class="d-inline"
            v-for="(lodging, index) in item.lodgings"
            :key="index"
          >
            <a
              class="fw-bold icon-link"
              :href="`${lodgingBaseUrl}/${lodging.id}/show`"
              v-if="lodging.id !== item.lodgings[item.lodgings.length - 1].id"
              >{{ toTitle(lodging.name) }}</a
            >
