...
<template>
    <div class="position-relative bg-white rounded-3 pt-4 h-100">
        ...
        <div v-if="(isEmpty(value) || value.length === 0) && !['number', 'boolean'].includes(typeof value)"
            :class="templateClass.property">
            <!-- todo: do something with this button...-->
            <Button icon-class-end="plus-circle-fill" color-class="secondary" class="d-block mx-auto" round-class="pill"
                label="Add" size="sm" />
        </div>
        ...
        <div v-else-if="typeof value === 'object'">
            ...
            <div v-if="name === 'reviews'" class="text-center">
                <span :class="valueDisplayClass">{{ getReviewAverage(value) }}{{ getReviewAverage(value) ? '/5' : ''
                }}</span>
            </div>
            ...
            <div v-else>
                <Button :icon-class-start="isCollapseOpen ? 'caret-up-fill' : 'caret-down-fill'" :label="value.length"
                    data-bs-toggle="collapse" :data-bs-target="`#collection-collapse-${randomSlug}`" :aria-expanded="false"
                    :aria-controls="`collection-collapse-${randomSlug}`" class="w-100 text-primary pb-0" color-class=""
                    @click="isCollapseOpen = !isCollapseOpen" size="lg" />
