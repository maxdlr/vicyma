...
import {
  ...
  toTitle,
  truncate,
} from "../../composable/formatter/string";
...
<template>
  <div class="position-relative bg-white rounded-3 pt-4 h-100">
    <div class="position-absolute top-0 start-0 ps-2 pt-1 opacity-50">
      <small>{{ name }}</small>
    </div>
    ...
    <div v-else-if="typeof value === 'boolean'" class="text-center">
      <span
        class="badge fs-6 rounded-pill"
        :class="value ? 'text-bg-success' : 'text-bg-secondary'"
      >
    ...
    <div v-else-if="typeof value === 'number'" :class="templateClass.property">
      <span
        v-if="name.toLowerCase().includes('price')"
        :class="valueDisplayClass"
        >{{ value }} FCFA</span
      >
    ...
    <div v-else-if="typeof value === 'object'">
      <div v-if="name === 'reviews'" class="text-center">
        <span :class="valueDisplayClass"
          >{{ getReviewAverage(value)
          }}{{ getReviewAverage(value) ? "/5" : "" }}</span
        >
      </div>
      ...
      <div v-else-if="value.length < 2">
        <div
          :class="templateClass.propertyListItem"
          v-if="
            ['string', 'number'].includes(
              typeof value[0][Object.keys(value[0])[1]],
            )
          "
        >
          <a
            :href="`/admin/${singularize(name)}/${value[0][Object.keys(value[0])[0]]}/show`"
            class="icon-link icon-link-hover text-primary"
            :class="valueDisplayClass"
          >
      ...
      <div v-else>
        <Button
          :icon-class-start="
            isCollapseOpen ? 'caret-up-fill' : 'caret-down-fill'
          "
          :label="value.length"
        ...
        <div class="collapse" :id="`collection-collapse-${randomSlug}`">
          <div v-for="item in value" :key="value">
            <div
              :class="templateClass.propertyListItem"
              v-if="
                ['string', 'number'].includes(typeof item[Object.keys(item)[1]])
              "
            >
              <a
                :href="`/admin/${singularize(name)}/${item[Object.keys(item)[0]]}/show`"
                class="icon-link icon-link-hover text-primary"
                :class="valueDisplayClass"
              >
