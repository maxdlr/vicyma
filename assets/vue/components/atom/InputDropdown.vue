...
<template>
    <div :id="`select-${label}`" v-on-click-outside="closeList" class="position-relative">
        <Button :class="isOpen ? 'border-bottom-0 bg-white' : ''" :color-class="`outline-${mainColorClass}`"
            ...
            :round-class="isOpen ? 'top-5 rounded-bottom-0' : '5'" class="w-100 position-relative z-2"
            @click.prevent="openList" @keydown.esc="closeList" />
        <Transition :name="SLIDE_DOWN">
            <div v-if="isOpen" class="position-absolute top-100 start-0 w-100 z-0">
                <ul
                    :class="`list-group list-group-flush rounded-bottom-5 border border-top-0 border-1 border-${mainColorClass}`">
                    <li v-if="!noEmpty" class="list-group-item list-group-item-action p-0" @click.prevent="select('')">
                        <Button :class="`w-100 text-${mainColorClass}`" color-class="" label="All" round-class="" />
                    </li>
                    <li v-for="(option, index) in options" :key="index" class="list-group-item list-group-item-action p-0">
                        <Button :class="`w-100 text-${mainColorClass}`"
