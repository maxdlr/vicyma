...
<template>
    <div v-if="!hideResultCount || !hideOrderBy" class="justify-content-between align-items-center d-flex">
        ...
        <Button v-if="!hideOrderBy" :icon-class-start="`caret-${isOrderReversed ? 'up' : 'down'}-fill`"
    ...
    <div v-for="(item) in items" v-else :key="item.id">
        ...
        <slot :item="item" name="row">
            <VDatatableRow :admin="admin" :exclude-properties="excludeFromRowProperties" :hide-empty="hideEmpty"
                :item="item" :max-cell-count-in-row="maxCellCountInRow">
